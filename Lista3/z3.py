    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            if x[i - 1] == y[j - 1]:
                ...
                insert_cost = dp[i][j - 1] + normalized_counts.get(x[j - 1], default_weight)
                delete_cost = dp[i - 1][j] + (1.0 / normalized_counts.get(x[i - 1], default_weight))
                replace_cost = dp[i - 1][j - 1] + get_key_distance(x[i - 1], y[j - 1]) + normalized_counts.get(y[j - 1], default_weight)
